...
//if we finished round1 then load a menu item for round2
if(getCookie("played") == "round1")
	menuText = new PIXI.Text("         Round 2!", titleStyle);
else
	menuText = new PIXI.Text("         Round 1!", titleStyle);
menuText.x = app.screen.width/2/2
menuText.y = app.screen.height/2-300+300
menuItems.push(menuText);
...
messages2 = [
	"Resetting game for round 2",
	"  Initalizing all objects ",
	"Reconnecting with players ",
	"Waiting for players 1/4.. ",
	"Waiting for players 2/4.  ",
	"Waiting for players 3/4.. ",
	"Waiting for players 3/4...",
...
function countDown2(){
	...
	if(countingDown)
		setTimeout(() =>{
			countDown()
			},1000 + ((messages2.length < 9) ? 0 : Math.random()*1000)
		)
}
...
function ending(){
	...
	const endStyleExtra = new PIXI.TextStyle({
	    fontFamily: 'Arial',
	    fontSize: 20,
	    // fontStyle: 'italic',
	    // fontWeight: 'bold',
	...
	menuText = new PIXI.Text("Please take note of your score.\nYou will play the game one more time", endStyleExtra);
	menuText.x = app.screen.width/2/2
	menuText.y = app.screen.height/2-300+400
	app.stage.addChild(menuText);
...
function ending2(){
	...
	menuText = new PIXI.Text("Please take note of your final score.\nYou will use these coins in the latter portion of the experiment", endStyleExtra);
	menuText.x = app.screen.width/2/2-80
	menuText.y = app.screen.height/2-300+450
	app.stage.addChild(menuText);
...
app.ticker.add((delta) => {
	...
	// Game over scene
	if(end){
		// meaning we are in round1
		if(getCookie("played") == "round1")
			ending()
		else
			ending2() //ending for round 2
